...
secondTp = Position(790, 173, 7), -- where the teleport that needs access sends the player
firstAid = 57019, -- actoin id of the first teleport
secondAid = 57020, -- actoin id of the second teleport
sto = 93119, -- storage that gives access
message = "Voce ainda nao liberou esse atalho. A melhor hunt de Sorcerer do game! Para isso, mate o boss quase impossivel." -- message if player dont have access
}
...
function firstTeleport.onStepIn(creature, item, position, fromPosition)
    ...
    if player then
        ...
        player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
				player:sendTextMessage(MESSAGE_STATUS, "Voce liberou o acesso a melhor hunt de Sorcerer do game!")
    end
return true
end
