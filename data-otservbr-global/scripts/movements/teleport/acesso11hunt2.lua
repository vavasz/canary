local c = {
    ...
    secondTp = Position(1288, 174, 7), -- where the teleport that needs access sends the player
firstAid = 57021, -- actoin id of the first teleport
secondAid = 57022, -- actoin id of the second teleport
sto = 93120, -- storage that gives access
message = "Voce ainda nao liberou esse atalho. A melhor hunt de Druid do game!. Para isso, mate o boss quase impossivel." -- message if player dont have access
}
...
function firstTeleport.onStepIn(creature, item, position, fromPosition)
    ...
    if player then
        ...
        player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
				player:sendTextMessage(MESSAGE_STATUS, "Voce liberou o acesso a melhor hunt de Druid do game!")
    end
return true
end
