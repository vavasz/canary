...
function portalreward.onStepIn(creature, item, position, fromPosition)
	...
	if text == "" then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voce nao derrotou o mal maior. Acabe com Sauron para receber seu premio!" .. text)
		player:teleportTo(fromPosition, true)
		return false
	end
end
...
portalreward:position({ x = 29, y = 1547, z = 7 })
